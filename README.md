...
- **Public Network Mode** (Settings) — a manual "I'm somewhere risky" switch for cafés, airports, shared computers, etc. Important to be honest about this: **no website can actually detect whether a WiFi network is safe** — browsers deliberately don't expose that to JavaScript. This isn't detection, it's a toggle you flip yourself, which then: caps auto-lock at 1 minute regardless of your normal setting, forces lock-on-focus-loss on, and hides the breach-check button (not because it's unsafe over HTTPS, just to cut unnecessary network activity while you're on a network you don't trust). There's also a one-time, Chromium-only *hint* — if `navigator.connection` reports you're on some kind of WiFi, it'll suggest turning the mode on — but that API can only see "WiFi vs. cellular vs. ethernet," never whether it's safe, and doesn't exist at all in Safari/Firefox. Treat it as a nudge, not a security signal.
- **Windows Hello support** — the biometric unlock (above) works with Windows Hello specifically as of a recent Windows update: you need Windows 11 25H2, build 26200.7840 or later (February 2026), plus a current Chrome, Edge, or Firefox. Older Windows/browser combinations genuinely don't support the underlying mechanism yet — that's a Microsoft/browser limitation, not something fixable from this app's side.
- **Security Levels (Low/Mid/High)** — a one-click preset for auto-lock duration, lock-on-blur, and Pulse Lock. Deliberately never touches Self-Destruct or wipe-on-missed-pulse at any level — a convenience shortcut should never be how an irreversible action gets switched on.
- **Dead Man's Switch / Pulse Lock** (off by default) — instead of only reacting to inactivity, demands an explicit "I'm still here" confirmation every N minutes regardless of mouse movement. On a missed pulse, it locks the vault (default) or — if you deliberately opt into it with a typed confirmation — wipes it. **Be honest with yourself about the false-positive rate before choosing wipe**: a bathroom break, a phone call, or your laptop sleeping looks identical to this feature as an actual threat. There's no way to tell them apart from JavaScript.
- **Self-Destruct on repeated failed unlocks** (off by default) — wipes the vault after N consecutive wrong passwords, on top of the escalating lockout everyone gets. Ships with **dry-run mode on by default** every time you enable it — it'll show you exactly when it would have triggered without deleting anything, so you can sanity-check your threshold before making it live. Turning dry-run off requires you to have already turned dry-run on and tested it at least once (the checkbox re-arms in dry-run every time you toggle the feature on).

### A genuinely important trade-off worth understanding before using either destructive feature
Both Self-Destruct and wipe-on-missed-pulse defend against **someone trying to get into your vault**. Neither does anything against **someone who just wants to destroy your data and doesn't care about reading it** — if anything, they make that easier: an attacker who can't guess your password can still ruin your day by entering wrong ones on purpose. For most personal threat models, you (tired, autocorrect, a toddler at the keyboard) are a far more likely trigger than a sophisticated attacker patiently brute-forcing against an already-aggressive lockout. Use the dry-run mode, actually test it, and think about whether the failure mode you're defending against is more likely than the failure mode you're creating.
...
```
SHA-256: 91e743c42c6beb3b3acb724ec82d2d22d7fbde025aabadd57187eb9000308047
```
